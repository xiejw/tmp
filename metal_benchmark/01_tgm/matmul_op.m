...
void matmulOpPrepareData(struct matmul_op* op) {
        ...
        size_t nb = sizeof(float) * len;

        op->buf_a = [op->dev
                newBufferWithLength: nb
                            options: MTLResourceStorageModeShared];
        op->buf_b = [op->dev
                newBufferWithLength: nb
                            options: MTLResourceStorageModeShared];

        generateRandomFloatData(op->buf_a, len);
        generateRandomFloatData(op->buf_b, len);
}
...
void matmulOpRun(struct matmul_op* op) {
        ...
        encodeOpCommand(op, computeEncoder);
        ...
        NSLog(@"op takes %g secs", end - start);
...
void encodeOpCommand(
        ...
        [computeEncoder setBytes: &len length:sizeof(int) atIndex:3];
...
void verifyResults(struct matmul_op *op) {
        ...
        for (size_t index = 0; index < len; index++) {
                if (a[index] != b[index]) {
                        NSLog(@"index: %zu - %f vs %f", index, a[index], b[index]);
                        if (count ++ > 10) {
