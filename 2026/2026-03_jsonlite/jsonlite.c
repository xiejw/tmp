...
static int encode_utf8(unsigned long cp, char *buf)
{
    if (cp <= 0x7F) {
        buf[0] = (char)cp;
        return 1;
    ...
    /* Slow path: unescape into buffer */
    {
        ...
        for (;;) {
            ...
            if (c == '\\') {
                ...
                switch (c) {
                case '"':  buf[buf_len++] = '"';  advance(p); break;
                case '\\': buf[buf_len++] = '\\'; advance(p); break;
                case '/':  buf[buf_len++] = '/';  advance(p); break;
                case 'b':  buf[buf_len++] = '\b'; advance(p); break;
                case 'f':  buf[buf_len++] = '\f'; advance(p); break;
                case 'n':  buf[buf_len++] = '\n'; advance(p); break;
                case 'r':  buf[buf_len++] = '\r'; advance(p); break;
                case 't':  buf[buf_len++] = '\t'; advance(p); break;
                case 'u': {
                    long cp;
                    advance(p); /* consume 'u' */
                    cp = parse_hex4(p);
                    ...
                    if (cp >= 0xD800 && cp <= 0xDBFF) {
                        ...
                        buf_len += (size_t)encode_utf8(full,
                                                       buf + buf_len);
