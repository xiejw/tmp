...
static const int  kBraceSize = 4;
static const char kScanFmt[] =
    "%63s %3s";  // widths = kFlagNameSize-1, kBraceSize-1
...
static FlagSpec *
DefFindSpec( FlagDef *def, const char *name )
{
...
bool
FlagDefParseDsl( FlagDef *def, const char *dsl_text, std::string *err_msg )
{
    def->count = 0;
        ...
        *end       = '\0';
        ...
        if ( line[0] != '\0' ) {
            if ( state == OUTSIDE ) {
                ...
                char brace[kBraceSize];
                int  matched = sscanf( line, kScanFmt, name, brace );
                if ( matched == 2 && strcmp( brace, "{" ) == 0 ) {
                    ...
                    snprintf( cur_spec->name, sizeof( cur_spec->name ), "%s",
                              name );
                    has_type = false;
                    state    = INSIDE;
                } else if ( line[0] != '\0' ) {
                    *err_msg =
                        std::string( "unexpected line outside block: " ) + line;
                    delete[] buf;
                    return true;
                }
            } else {
                ...
                if ( strcmp( line, "}" ) == 0 ) {
                    ...
                    if ( colon == nullptr ) {
                        *err_msg =
                            std::string( "expected 'key: value' but got: " ) +
                            line;
                        delete[] buf;
                        return true;
                    }
                    *colon    = '\0';
                    ...
                    if ( strcmp( key, "type" ) == 0 ) {
                        ...
                        if ( strcmp( val, "true" ) == 0 ) {
                            ...
                            *err_msg =
                                std::string( "invalid value for required: " ) +
                                val;
                            delete[] buf;
                            return true;
                        }
                        ...
                        if ( !has_type ) {
                            *err_msg =
                                "default_value requires type to be declared "
                                "first";
                            delete[] buf;
                            return true;
                        }
                        ...
                        snprintf( cur_spec->default_str,
                                  sizeof( cur_spec->default_str ), "%s", val );
