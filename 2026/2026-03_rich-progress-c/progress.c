...
struct sample { double t; int64_t count; };
...
static void render_bar(void)
{
    ...
    double pct     = (s_total > 0) ? (double)s_current / (double)s_total : 0.0;
    if (pct > 1.0) pct = 1.0;
    ...
    if (s_current > 0 && pct < 1.0) {
        if (s_sample_count >= 2) {
            int oldest_idx = (s_sample_head - s_sample_count + MA_SIZE) % MA_SIZE;
            int newest_idx = (s_sample_head - 1 + MA_SIZE) % MA_SIZE;
            double dt      = s_samples[newest_idx].t - s_samples[oldest_idx].t;
            int64_t dc     = s_samples[newest_idx].count - s_samples[oldest_idx].count;
            if (dt > 0.0 && dc > 0) {
